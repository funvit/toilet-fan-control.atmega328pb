...
void TroykaOLED::setFont(const uint8_t *fontData) {
  ...
  //  сохраняем ширину символов выбранного шрифта читая её из 0 байта массива по
  //  указателю fontData
  _font.width = pgm_read_byte(&fontData[0]);
  // сохраняем высоту символов выбранного шрифта читая её из 1 байта массива по
  // указателю fontData
  _font.height = pgm_read_byte(&fontData[1]);
  // сохраняем код первого симола выбран. шрифта читая его из 2 байта массива по
  // указателю fontData
  _font.firstSymbol = pgm_read_byte(&fontData[2]);
  // сохраняем количество символов в выбр шрифте читая их  из 3 байта массива по
  // указателю fontData
  _font.sumSymbol = pgm_read_byte(&fontData[3]);
  //  устанавливаем флаг выбора шрифта
  _font.setFont = true;
  // определяем позицию бита указывающего количество пустых интервалов в массиве
  // шрифта.
  uint16_t i =
      (uint16_t)_font.sumSymbol * _font.width * _font.height / 8 + 0x04;
  // определяем количество пустых интервалов в массиве шрифта.
  uint16_t j = pgm_read_byte(&fontData[i]);
  // указываем что первый пустой интервал в массиве шрифта находится после
  // символа с кодом (0xFF) и состоит из 0 символов
  _font.startSpace[0] = 0xFF;
  _font.sumSpace[0] = 0;
  // указываем что второй пустой интервал в массиве шрифта находится после
  // символа с кодом (0xFF) и состоит из 0 символов
  _font.startSpace[1] = 0xFF;
  _font.sumSpace[1] = 0;
  // указываем что третий пустой интервал в массиве шрифта находится после
  // символа с кодом (0xFF) и состоит из 0 символов
  _font.startSpace[2] = 0xFF;
  _font.sumSpace[2] = 0;
  // если количество пустых интервалов больше 0
  // сохраняем начало первого пустого интервала символов и размер первого
  // пустого интервала символов
  if (j > 0) {
  ...
  // сохраняем начало второго пустого интервала символов и размер второго
  // пустого интервала символов
  if (j > 1) {
  ...
  // сохраняем начало третьего пустого интервала символов и размер третьего
  // пустого интервала символов
  if (j > 2) {
...
void TroykaOLED::print(int32_t num, int x, int y, uint8_t base) {
  ...
  // i = количество разрядов + 2, j = множитель кратный основанию системы
  // счисления
  int8_t i = 2;
  int32_t j = 1;
  while (num / j) {
...
void TroykaOLED::print(uint32_t num, int x, int y, uint8_t base) {
  // определяем количество разрядов числа
  // i = количество разрядов + 1, j = множитель кратный основанию системы
  // счисления
  int8_t i = 1;
  uint32_t j = 1;
  while (num / j) {
...
void TroykaOLED::drawRect(int x1, int y1, int x2, int y2, bool fill,
                          uint8_t color) {
  if (fill) {
    if (x1 < x2) {
      for (int x = x1; x <= x2; x++) {
...
void TroykaOLED::drawCircle(int x, int y, uint8_t r, bool fill, uint8_t color) {
  ...
  while (x1 < y1 + 1) {
    if (fill) {
      // прорисовываем горизонтальные линии вверху круга (между точками 3 и 1
      // дуг)
      _drawLine(x - x1, y - y1, x + x1, y - y1, color);
      // прорисовываем горизонтальные линии внизу  круга (между точками 4 и 2
      // дуг)
      _drawLine(x - x1, y + y1, x + x1, y + y1, color);
      // прорисовываем горизонтальные линии выше середины круга (между точками 7
      // и 5 дуг)
      _drawLine(x - y1, y - x1, x + y1, y - x1, color);
      // прорисовываем горизонтальные линии выше середины круга (между точками 8
      // и 6 дуг)
      _drawLine(x - y1, y + x1, x + y1, y + x1, color);
    } else {
    ...
    if (p >= 0) {
      // сдвигаем её вниз на y1 * 2 (каждый такой сдвиг провоцирет смещение
      // точки y1 первой дуги вниз)
      y1--;
      p -= y1 * 2;
    }
    // с каждым проходом цикла, смещаем точку x1 первой дуги влево и находим
    // новую координату параболы p
    p++;
    x1++;
    p += x1 * 2;
  }
...
bool TroykaOLED::getPixel(int x, int y) {
  ...
  // определяем номер бита в найденном байте, который соответсвует искомому
  // пикселю
  uint8_t numBit = y % 8;
  // возвращаем цвет пикселя из бита numBit элемента numByte массива
  // _bufferDisplay
...
void TroykaOLED::_print(char *data, int x, int y) {
  // если шрифт не выбран или его высота не кратна 8 пикселям, то выходим из
  // функции
  ...
  // пересчитываем количество колонок которое занимают выводимые символы, с
  // учётом начальной позиции
  if (_numX + len > _width) {
  ...
  for (int8_t p = 0; p < _font.height / 8; p++) {
    ...
    for (uint8_t n = 0; n < (len / _font.width); n++) {
      ...
      num = uint8_t(data[n]);
      // если в массиве символов, до кода текущего символа, имеется пустой
      // интервал уменьшаем код текущего символа на количество символов в пустом
      // интервале
      ...
      // если в массиве символов, до кода текущего символа, имеется пустой
      // интервал то уменьшаем код текущего символа на количество символов в
      // пустом интервале
      ...
      num *= _font.height / 8;
      // добавляем количество колонок данного символа, которые уже были выведены
      // на предыдущих страницах
      num += p * _font.width;
      // добавляем количество байт в начале массива шрифта, которые не являются
      // байтами символов
      num += 0x04;
      ...
      for (uint8_t k = 0; k < _font.width; k++) {
        ...
        for (uint8_t b = 0; b < 8; b++) {
          // начальная колонка всего текста + (количество выведенных символов *
          // ширина символов) + номер байта текущего символа
          x1 = _numX + n * _font.width + k;
          // нижняя строка текста - высота симолов + количество уже выведенных
          // страниц + номер бита байта текущего символа + 1
          y1 = _numY + p * 8 + b;
          ...
          if (_font.invert) {
            // если установлен фон текста или точка стоит на букве (а не на
            // фоне)
...
//  преобразуем цифры 0-9 в символ с кодом 48-57, а цифры 10-15 в символ с кодом
//  65-71
...
void TroykaOLED::_drawPixel(int x, int y, uint8_t color) {
  ...
  uint8_t p = y / 8;
  // определяем номер байта массива _bufferDisplay в котором требуется
  // прорисовать пиксель
  uint16_t numByte = (p * 128) + x;
  // определяем номер бита в найденном байте, который соответсвует рисуемому
  // пикселю
  uint8_t numBit = y % 8;
  switch (color) {
  case WHITE:
...
void TroykaOLED::printWrapping(String str, int x, int y, bool keepX) {
  char data[str.length() + 1];
  str.toCharArray(data, str.length() + 1);
  this->printWrapping(data, x, y, keepX);
}
/**
 * print() с переносом строки
 *
 ...
 * @param keepX сохранять позицию по X при переносе строки
 */
void TroykaOLED::printWrapping(char *str, int x, int y, bool keepX) {

  char *s = _codingCP866(str);
  ...
  while (idx < len) {
    ...
    for (uint8_t i = 0; i <= charsPerRow - round(1.0 * x / _font.width + 0.49);
         i++) {
      if (idx + i > len) {
        break;
      }
