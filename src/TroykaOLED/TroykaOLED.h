...
// установка контрастности, за данной командой должен быть отправлен байт
// контрастности от 00 до FF (по умолчанию 0x7F)
...
// настройка непрерывной горизонтальной прокрутки вправо. За данной командой
// должны быть отправлены 6 байт настроек
#define SSD1306_SCROLL_HORIZONTAL_RIGHT 0x26
// настройка непрерывной горизонтальной прокрутки влево. За данной командой
// должны быть отправлены 6 байт настроек
#define SSD1306_SCROLL_HORIZONTAL_LEFT 0x27
// настройка непрерывной диагональной прокрутки вправо. За данной командой
// должны быть отправлены 5 байт настроек
#define SSD1306_SCROLL_DIAGONAL_RIGHT 0x29
// настройка непрерывной диагональной прокрутки влево. За данной командой должны
// быть отправлены 5 байт настроек
#define SSD1306_SCROLL_DIAGONAL_LEFT 0x2A
// настройка непрерывной вертикальной прокрутки. За данной командой должны быть
// отправлены 2 байта настроек
#define SSD1306_SCROLL_VERTICAL 0xA3

// установка младшей части адреса колонки на странице
// у данной команды младщие четыре бита должны быть изменены на младшие биты
// адреса комадна предназначена только для режима страничной адресации
#define SSD1306_ADDR_COLUMN_LBS 0x00
// установка старшей части адреса колонки на странице
// у данной команды младщие четыре бита должны быть изменены на старшие биты
// адреса комадна предназначена только для режима страничной адресации
#define SSD1306_ADDR_COLUMN_MBS 0x10
// выбор режима адресации
// за данной командой должен быть отправлен байт младшие биты которого
// определяют режим: 00-горизонтальная (с переходом на новую страницу (строку))
...
// за данной командой должны быть отправлены два байта: начальный адрес и
// конечный адрес так можно определить размер экрана в колонках по ширине
#define SSD1306_ADDR_COLUMN 0x21
// установка адреса страницы
// за данной командой должны быть отправлены два байта: начальный адрес и
// конечный адрес так можно определить размер экрана в строках по высоте
#define SSD1306_ADDR_PAGE 0x22
// установка номера страницы которая будет выбрана для режима страничной
// адресации у данной команды младщие три бита должны быть изменены на номер
// страницы комадна предназначена только для режима страничной адресации
#define SSD1306_ADDR_ONE_PAGE 0xB0

// установить начальный адрес ОЗУ (смещение памяти)
// у данной команды младщие шесть битов должны быть изменены на начальный адрес
// ОЗУ
...
// установить multiplex ratio (количество используемых выводов COM для вывода
// данных на дисплей) за данной командой должен быть отправлен один байт с
// указанием количества COM выводов (от 15 до 63)
...
// за данной командой должен быть отправлен один байт с указанием вертикального
// сдвига чтения выходов COM (от 0 до 63).
...
// за данной командой должен быть отправлен один байт, старшие четыре бита
// которого определяют частоту, а младшие делитель
#define SSD1306_SET_DISPLAY_CLOCK 0xD5
// установить фазы DC/DC преобразователя
// за данной командой должен быть отправлен один байт, старшие четыре бита
// которого определяют период, а младшие период
#define SSD1306_SET_PRECHARGE_PERIOD 0xD9
// установить VcomH (влияет на яркость)
// за данной командой должен быть отправлен один байт, старшие четыре бита
// которого определяют напряжение пример: 0000 - VcomH=0.65Vcc, 0010 -
// VcomH=0.77Vcc, 0011 - VcomH=0.83Vcc и т.д.
...
// этот бит указывает что после следующего байта (команды или данных) будет
// следовать (если будет): 0-байт данных или 1-новый командный байт
...
public:
  ...
  void print(int8_t n, int x = OLED_THIS, int y = OLED_THIS,
             uint8_t base = DEC);
  void print(uint8_t n, int x = OLED_THIS, int y = OLED_THIS,
             uint8_t base = DEC);
  void print(int16_t n, int x = OLED_THIS, int y = OLED_THIS,
             uint8_t base = DEC);
  void print(uint16_t n, int x = OLED_THIS, int y = OLED_THIS,
             uint8_t base = DEC);
  void print(int32_t n, int x = OLED_THIS, int y = OLED_THIS,
             uint8_t base = DEC);
  void print(uint32_t n, int x = OLED_THIS, int y = OLED_THIS,
             uint8_t base = DEC);
  void print(double n, int x = OLED_THIS, int y = OLED_THIS,
             uint8_t digits = 2);
  ...
  void drawRect(int x1, int y1, int x2, int y2, bool fill = false,
                uint8_t color = WHITE);
  void drawCircle(int x, int y, uint8_t r, bool fill = false,
                  uint8_t color = WHITE);
  void drawImage(const uint8_t *image, int x = OLED_THIS, int y = OLED_THIS,
                 uint8_t mem = IMG_ROM);
  ...
  size_t getTextLength(String str);
  void printWrapping(String s, int x, int y, bool keepX);
  void printWrapping(char *s, int x, int y, bool keepX);
...
private:
  ...
  void _sendBuffer();
  size_t _getTextLength(String str);
};
